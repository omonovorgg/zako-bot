...
async def init_db():
    ...
    async with p.acquire() as c:
        ...
        CREATE TABLE IF NOT EXISTS zako_shop_items(
            id BIGSERIAL PRIMARY KEY, kind TEXT NOT NULL CHECK(kind IN ('gift','coin_pack')), title TEXT NOT NULL, description TEXT NOT NULL DEFAULT '',
            coins_price BIGINT NOT NULL DEFAULT 0, stars_price INTEGER NOT NULL DEFAULT 0, active BOOLEAN NOT NULL DEFAULT true, sort_order INTEGER NOT NULL DEFAULT 0
        );
        ...
        SELECT s.telegram_id,s.category,s.pct,s.attempt_id,(s.created_at AT TIME ZONE 'UTC')::date,s.created_at
...
async def admin_callback(uid,mode):
    ...
    if mode=="prizes_bulk":
        ...
        for line in text.splitlines():
            ...
            a,b=line.split("|",1)
            try:place=int(a.strip())
            except:continue
            if 1<=place<=10:rows.append((place,b.strip()))
        if len(rows)<10:return await tg("sendMessage",{"chat_id":uid,"text":"❌ 1 dan 10 gacha bo‘lgan 10 qatorni yuboring: 1|Sovrin ... 10|Sovrin"})
        p=await db()
        async with p.acquire() as c:
            period=st["period"]
            for place,prize in rows:
                if prize: await c.execute("INSERT INTO zako_rank_prizes(period,place,prize) VALUES($1,$2,$3) ON CONFLICT(period,place) DO UPDATE SET prize=EXCLUDED.prize",period,place,prize)
                else: await c.execute("DELETE FROM zako_rank_prizes WHERE period=$1 AND place=$2",period,place)
        return await tg("sendMessage",{"chat_id":uid,"text":f"✅ {'Haftalik' if period=='week' else 'Oylik'} TOP 10 sovrinlari saqlandi. App reytingida ko‘rinadi."})
    if mode=="shop_add":
        parts=[x.strip() for x in text.split("|",3)]
        if len(parts)!=4:return await tg("sendMessage",{"chat_id":uid,"text":"❌ Format: Nomi|Tavsif|Coin|Stars"})
        try:coins=int(parts[2]);stars=int(parts[3])
        except:return await tg("sendMessage",{"chat_id":uid,"text":"❌ Coin va Stars raqam bo‘lsin."})
        kind=st["kind"]
        ...
        p=await db()
        async with p.acquire() as c:await c.execute("INSERT INTO zako_shop_items(kind,title,description,coins_price,stars_price) VALUES($1,$2,$3,$4,$5)",kind,parts[0],parts[1],coins,stars)
        return await tg("sendMessage",{"chat_id":uid,"text":"✅ Shop elementi qo‘shildi."})
    if mode=="shop_edit":
        parts=[x.strip() for x in text.split("|",4)]
        if len(parts)!=5:return await tg("sendMessage",{"chat_id":uid,"text":"❌ Format: Nomi|Tavsif|Coin|Stars|ON/OFF"})
        try:coins=int(parts[2]);stars=int(parts[3])
        except:return await tg("sendMessage",{"chat_id":uid,"text":"❌ Coin va Stars raqam bo‘lsin."})
        active=parts[4].upper() in {"ON","1","TRUE","HA"};p=await db()
        async with p.acquire() as c:res=await c.execute("UPDATE zako_shop_items SET title=$2,description=$3,coins_price=$4,stars_price=$5,active=$6 WHERE id=$1",st["item_id"],parts[0],parts[1],coins,stars,active)
    ...
    if mode=="top":
        p=await db()
        async with p.acquire() as c:
            rows=await c.fetch("""SELECT u.first_name,u.username,COALESCE(SUM(s.pct),0)::int score FROM zako_users u LEFT JOIN zako_scores s ON s.telegram_id=u.telegram_id GROUP BY u.telegram_id ORDER BY score DESC,u.telegram_id LIMIT 10""")
        lines=["🏆 <b>TOP 10</b>",""]
    ...
    if mode in {"prizesw","prizesm"}:
        period="week" if mode=="prizesw" else "month";admin_state[uid]={"mode":"prizes_bulk","period":period}
        p=await db()
        ...
        lines=[f"🎁 <b>{title} TOP 10 sovrinlari</b>","","Har qator: <code>1|100 000 so‘m</code>","Bo‘sh qoldirish = o‘chirish",""]
        lines += [f"{int(x['place'])}|{htmlmod.escape(x['prize'])}" for x in rows]
        return await tg("sendMessage",{"chat_id":uid,"text":"\n".join(lines)+"\n\n10 qatorni yuboring.","parse_mode":"HTML"})
    if mode=="shop":
        p=await db()
        async with p.acquire() as c:rows=await c.fetch("SELECT id,kind,title,coins_price,stars_price,active FROM zako_shop_items ORDER BY id")
        lines=["🛍 <b>Coin Shop boshqaruvi</b>",""]
        for x in rows:lines.append(f"#{x['id']} {'🎁' if x['kind']=='gift' else '🪙'} {htmlmod.escape(x['title'])} — {int(x['coins_price'])} Coin / {int(x['stars_price'])} ⭐ — {'ON' if x['active'] else 'OFF'}")
        lines += ["", "➕ Yangi sovg‘a: /addgift", "➕ Coin paketi: /addcoin", "✏️ Tahrirlash: /editshop ID", "🗑 O‘chirish: /delshop ID"]
        return await tg("sendMessage",{"chat_id":uid,"text":"\n".join(lines),"parse_mode":"HTML"})
    if mode=="cleanup":
        p=await db()
        async with p.acquire() as c:
...
async def admin_flow(m):
    ...
    if uid in ADMIN_IDS and text in {"/addgift","/addcoin"}:
        admin_state[uid]={"mode":"shop_add","kind":"gift" if text=="/addgift" else "coin_pack"}
        return await tg("sendMessage",{"chat_id":uid,"text":"Yuboring: <code>Nomi|Tavsif|Coin|Stars</code>\n\nGift uchun Stars=0. Coin paketi uchun Coin miqdori va Stars narxi yoziladi.","parse_mode":"HTML"})
    if uid in ADMIN_IDS and text.startswith("/editshop "):
        ...
        return await tg("sendMessage",{"chat_id":uid,"text":"Yangi qiymat: <code>Nomi|Tavsif|Coin|Stars|ON/OFF</code>","parse_mode":"HTML"})
...
async def handle_update(upd):
    if "message" in upd:
        ...
        if m.get("successful_payment"):
            sp=m["successful_payment"];payload=sp.get("invoice_payload","");p=await db()
            async with p.acquire() as c:
...
async def api_score(r:FRequest):
    u=validate_init(r.headers.get("X-Telegram-Init-Data",""));await save_user(u);b=await r.json();cat=b.get("category");attempt=str(b.get("attempt_id","")).strip()
    try:pct=int(b.get("pct"))
    except:raise HTTPException(400,"Ball noto‘g‘ri.")
    if cat not in DATA["core"] or not attempt or len(attempt)>100 or not 0<=pct<=100:raise HTTPException(400,"Natija noto‘g‘ri.")
    uid=int(u["id"]);p=await db()
    async with p.acquire() as c:
        inserted=await c.fetchrow("""INSERT INTO zako_daily_scores(telegram_id,category,pct,attempt_id) VALUES($1,$2,$3,$4) ON CONFLICT(telegram_id,category,score_day) DO NOTHING RETURNING id""",uid,cat,pct,attempt)
        if not inserted:return {"ok":True,"ranked":False,"already_done":True}
        xp_gain=100+pct
        await c.execute("UPDATE zako_users SET xp=xp+$2,last_seen=now() WHERE telegram_id=$1",uid,xp_gain)
        await c.execute("""UPDATE zako_users SET streak=CASE WHEN streak_day=CURRENT_DATE THEN streak WHEN streak_day=CURRENT_DATE-1 THEN streak+1 ELSE 1 END,streak_day=CURRENT_DATE WHERE telegram_id=$1""",uid)
    return {"ok":True,"ranked":True,"xp_gain":xp_gain}
...
async def wallet(r:FRequest):
    ...
    async with p.acquire() as c:row=await c.fetchrow("SELECT xp,coins,streak FROM zako_users WHERE telegram_id=$1",int(u["id"]))
    return {"xp":int(row["xp"]),"coins":int(row["coins"]),"streak":int(row["streak"])}

@app.get("/api/shop")
async def shop_api(r:FRequest):
    u=validate_init(r.headers.get("X-Telegram-Init-Data",""));await save_user(u);p=await db()
    async with p.acquire() as c:rows=await c.fetch("SELECT id,kind,title,description,coins_price,stars_price FROM zako_shop_items WHERE active=true ORDER BY kind,sort_order,id")
    return {"items":[{"id":int(x["id"]),"kind":x["kind"],"title":x["title"],"description":x["description"],"coins_price":int(x["coins_price"]),"stars_price":int(x["stars_price"])} for x in rows]}
...
async def shop_redeem(r:FRequest):
    ...
    async with p.acquire() as c:
        async with c.transaction():
            item=await c.fetchrow("SELECT * FROM zako_shop_items WHERE id=$1 AND active=true AND kind='gift' FOR UPDATE",item_id)
            if not item:raise HTTPException(404,"Sovg‘a topilmadi.")
            row=await c.fetchrow("SELECT coins FROM zako_users WHERE telegram_id=$1 FOR UPDATE",uid);price=int(item["coins_price"])
            if int(row["coins"])<price:raise HTTPException(400,"Coin yetarli emas.")
            await c.execute("UPDATE zako_users SET coins=coins-$2 WHERE telegram_id=$1",uid,price)
            await c.execute("INSERT INTO zako_redemptions(telegram_id,item_id,item_title,coins_spent) VALUES($1,$2,$3,$4)",uid,item_id,item["title"],price)
    try:await tg("sendMessage",{"chat_id":next(iter(ADMIN_IDS)),"text":f"🎁 <b>Coin Shop buyurtmasi</b>\n\n👤 User: <code>{uid}</code>\n🎁 {htmlmod.escape(item['title'])}\n🪙 {price} Coin","parse_mode":"HTML"})
    except Exception:pass
    return {"ok":True,"message":"Sovg‘a buyurtma qilindi."}

@app.get("/api/ranking")
async def ranking(period:str="week"):
    period=period if period in {"week","month"} else "week";p=await db();where="score_day>=CURRENT_DATE-INTERVAL '6 days'" if period=="week" else "score_day>=date_trunc('month',CURRENT_DATE)::date"
    async with p.acquire() as c:
