...
async def init_db():
    ...
    async with p.acquire() as c:
        ...
        await c.execute("INSERT INTO zako_shop_items(kind,title,description,coins_price,xp_price,stars_price,sort_order) SELECT 'frame','⚡ Energy Frame','Yorqin energiya ramkasi',120,0,0,20 WHERE NOT EXISTS(SELECT 1 FROM zako_shop_items WHERE kind='frame' AND title='⚡ Energy Frame')")
        await c.execute("INSERT INTO zako_shop_items(kind,title,description,coins_price,xp_price,stars_price,sort_order) SELECT 'frame','🔥 Flame Frame','Harakatlanuvchi olov effekti',250,0,0,21 WHERE NOT EXISTS(SELECT 1 FROM zako_shop_items WHERE kind='frame' AND title='🔥 Flame Frame')")
        await c.execute("INSERT INTO zako_shop_items(kind,title,description,coins_price,xp_price,stars_price,sort_order) SELECT 'frame','💎 Diamond Frame','Yaltirab turuvchi premium effekt',500,0,0,22 WHERE NOT EXISTS(SELECT 1 FROM zako_shop_items WHERE kind='frame' AND title='💎 Diamond Frame')")
        await c.execute("INSERT INTO zako_shop_items(kind,title,description,coins_price,xp_price,stars_price,sort_order) SELECT 'frame','👑 Royal Frame','Eng yuqori darajadagi animatsion ramka',1000,0,0,23 WHERE NOT EXISTS(SELECT 1 FROM zako_shop_items WHERE kind='frame' AND title='👑 Royal Frame')")
    return p
...
async def admin_flow(m):
    ...
    if mode=="shop_add":
        ...
        if len(parts)==4:title,desc,coins_s,stars_s=parts;xp_s="0"
        elif len(parts)==5:title,desc,xp_s,coins_s,stars_s=parts
        else:
            admin_state[uid]=st;return await tg("sendMessage",{"chat_id":uid,"text":"❌ Format: Nomi|Tavsif|XP|Coin|Stars"})
        try:
            xp=int(xp_s);coins=int(coins_s);stars=int(stars_s)
            if min(xp,coins,stars)<0:raise ValueError
        except:
            admin_state[uid]=st;return await tg("sendMessage",{"chat_id":uid,"text":"❌ XP, Coin va Stars 0 yoki undan katta son bo‘lsin."})
        kind=st.get("kind","gift")
        if not title:return await tg("sendMessage",{"chat_id":uid,"text":"❌ Nomi bo‘sh bo‘lmasin."})
...
async def ranking(period:str="week"):
    ...
    async with p.acquire() as c:
        rows=await c.fetch(f"SELECT u.telegram_id,u.first_name,u.username,u.frame_id,SUM(s.pct)::int score FROM zako_users u JOIN zako_daily_scores s ON s.telegram_id=u.telegram_id WHERE {where} GROUP BY u.telegram_id,u.frame_id ORDER BY score DESC,u.telegram_id LIMIT 10")
        prizes=await c.fetch("SELECT place,prize FROM zako_rank_prizes WHERE period=$1 ORDER BY place",period)
    return {"period":period,"rows":[{"id":int(x["telegram_id"]),"name":x["first_name"] or x["username"] or "ZAKO","score":int(x["score"]),"frame_id":int(x["frame_id"]) if x["frame_id"] else None} for x in rows],"prizes":[{"place":int(x["place"]),"prize":x["prize"]} for x in prizes]}
...
async def coin_manual(r:FRequest):
    ...
    async with p.acquire() as c:
        ...
        else:await c.execute("INSERT INTO zako_coin_orders(telegram_id,token,coins,amount_uzs) VALUES($1,$2,$3,$4)",uid,token,COIN_MANUAL_COINS,COIN_MANUAL_AMOUNT)
    return {"ok":True,"coins":COIN_MANUAL_COINS,"amount_uzs":COIN_MANUAL_AMOUNT,"bot_url":f"https://t.me/{BOT_USERNAME}?start=coinpay_{quote(token,safe='')}"}
