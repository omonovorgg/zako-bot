...
async def init_db():
    ...
    async with p.acquire() as c:
        ...
        CREATE TABLE IF NOT EXISTS zako_scores(
            ...
            telegram_id BIGINT NOT NULL REFERENCES zako_users(telegram_id) ON DELETE CASCADE,
        ...
        await c.execute("""
        CREATE TABLE IF NOT EXISTS zako_settings(
            k TEXT PRIMARY KEY,
            v TEXT NOT NULL
        )""")
        await c.execute("""
        CREATE TABLE IF NOT EXISTS zako_challenges(
            ...
            creator_id BIGINT NOT NULL REFERENCES zako_users(telegram_id) ON DELETE CASCADE,
            kind TEXT NOT NULL CHECK(kind IN ('love','friend')),
            q_ids JSONB NOT NULL,
            answers JSONB NOT NULL,
            used BOOLEAN NOT NULL DEFAULT false,
            created_at TIMESTAMPTZ NOT NULL DEFAULT now()
        )""")
        await c.execute("CREATE INDEX IF NOT EXISTS zako_scores_user_idx ON zako_scores(telegram_id)")
        await c.execute("CREATE INDEX IF NOT EXISTS zako_scores_created_idx ON zako_scores(created_at)")
        await c.execute("CREATE INDEX IF NOT EXISTS zako_challenges_created_idx ON zako_challenges(created_at)")
...
async def admin_flow(m):
    ...
    if not st:return
    mode=st.get("mode");text=(m.get("text") or "").strip()
    if mode=="channel":
...
async def api_score(r:FRequest):
    ...
    cat=b.get("category");attempt=str(b.get("attempt_id","")).strip()
    ...
    p=await db()
    async with p.acquire() as c:
        await c.execute("""INSERT INTO zako_scores(telegram_id,category,pct,attempt_id) VALUES($1,$2,$3,$4) ON CONFLICT(attempt_id) DO NOTHING""",int(u["id"]),cat,pct,attempt)
    return {"ok":True}
...
async def ranking():
    p=await db()
    async with p.acquire() as c:
        rows=await c.fetch("""SELECT u.first_name,u.username,COALESCE(SUM(s.pct),0)::int score FROM zako_users u LEFT JOIN zako_scores s ON s.telegram_id=u.telegram_id GROUP BY u.telegram_id ORDER BY score DESC,u.telegram_id LIMIT 50""")
    return {"rows":[{"name":r["first_name"] or r["username"] or "ZAKO","score":int(r["score"])} for r in rows]}

@app.get("/api/me")
async def me(r:FRequest):
    u=validate_init(r.headers.get("X-Telegram-Init-Data",""));await save_user(u);p=await db()
    async with p.acquire() as c:
        gs=await c.fetchval("SELECT COALESCE(SUM(pct),0)::int FROM zako_scores WHERE telegram_id=$1",int(u["id"]))
        best=await c.fetchval("SELECT COALESCE(MAX(pct),0)::int FROM zako_scores WHERE telegram_id=$1",int(u["id"]))
        rows=await c.fetch("SELECT category,COALESCE(AVG(pct),0)::int pct FROM zako_scores WHERE telegram_id=$1 GROUP BY category",int(u["id"]))
    return {"global_score":int(gs or 0),"best_score":int(best or 0),"categories":{r["category"]:int(r["pct"]) for r in rows}}
...
async def startup():
    ...
    wh=await tg("setWebhook",{"url":PUBLIC_URL+"/telegram/webhook","secret_token":WEBHOOK_SECRET,"allowed_updates":["message","callback_query"]})
